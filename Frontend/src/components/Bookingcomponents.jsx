...
export default function DateValidationDisablePast() {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const response = await axios.post(`/api/v1/bookings/bookandrequest?userId=${userId}&workerId=${workerId}`, requestData);
    ...
    const fetchData = async () => {
      try {
        const [workerResponse, userResponse] = await Promise.all([
          axios.get(`/api/v1/workers/work?id=${id}`),
          axios.get(`/api/v1/users/getThatUser?id=${userId}`)
        ]);

        console.log(workerResponse.data);
        setData(workerResponse.data);
        localStorage.setItem('workerId', workerResponse.data._id);

        console.log(userResponse.data);
        setDatas(userResponse.data);
        setAddress(userResponse.data.address_user);
      } catch (error) {
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <>
          <UserCard data={datas} /> {/* Pass datas to UserCard component */}
          <h4>Choose Date:</h4>
...
//     const fetchData = async () => {
//       try {
//         const response = await axios.get(`/api/v1/users/getThatUser?id=${userId}`);
//         console.log(response.data);
